...
void IRAM_ATTR epd_draw_image_frame(Rect_t area, uint8_t *data, DrawMode_t mode, uint8_t k)
{
    SemaphoreHandle_t fetch_sem = xSemaphoreCreateBinary();
    SemaphoreHandle_t feed_sem = xSemaphoreCreateBinary();
    OutputParams p1 = {
...
static void IRAM_ATTR provide_out(OutputParams *params)
{
    ...
    if (params->rebuild_lut)
    {
        reset_lut(conversion_lut, params->mode);
        for (int32_t i = 0; i <= params->frame; i++)
        {
            update_LUT(conversion_lut, (uint8_t)i, params->mode);
        }
    }
    else
    {
